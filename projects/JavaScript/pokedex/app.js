...
const URL_LOCALE = "http://localhost";
...
const filterPokemon = (type = ALL_TYPE, data) => {
    ...
    if (type !== ALL_TYPE) {
        filteredData = data.filter((pokemon) =>
            pokemon.typeList.includes(type)
        );
    } else {
        filteredData = data;
    }
...
const searchPokemon = (valueInput, data) => {
    ...
    if (value === "" || value === '"') {
        ...
        lis.forEach((li) => {
            let typeFormatted =
                '"' + li.attributes[1].value.toLowerCase() + '"';
...
const requestData = (type, action = BY_BUTTON, value) => {
    ...
    if (document.location.origin === URL_LOCALE) {
        URL =
            URL_LOCALE +
            "/ascenseur301/projects/JavaScript/pokedex/data/pokedex.json";
    } else if (document.location.origin === URL_DEPLOY) {
        URL = URL_DEPLOY + "/JavaScript/filtreJS/data/pokedex.json";
    } else {
